...
- Dashboard architecture (2026-05-28, uncommitted): Dashboard is read-only "awareness" only (identity + PRODUCTION/SANDBOX badge from `GET /qbo/status` `environment`, live read-only counts from `GET /company/snapshot`, lab footprint from checkpoints/issuepack-runs/audit, jump-offs). Write actions (seed/generate/issue-pack run) live on a separate `/lab` "Lab Tools" page. Production write guard: `backend/src/middleware/productionGuard.js` `requireProductionConfirm` is applied (after `authenticate`, before handler) to `seed POST /start`, `generate POST /start`, `issuepacks POST /:slug/run`; in production it returns HTTP 412 unless `req.body.confirmProduction === true` (strict boolean), no-op in sandbox. Frontend `ProductionGuardDialog` collects that intent (ack checkbox + typed "PRODUCTION") only when environment is production (unknown env defaults to production friction). `detectTier` in `company.js` now best-effort detects 'Advanced' from `CompanyInfo.NameValue` (no documented tier field; falls back to feature heuristics). Reviewed by qbo-safety-reviewer (SAFE) + implementation-reviewer (blocker fixed). Follow-up: AI plan-execution writes are NOT behind the guard.
