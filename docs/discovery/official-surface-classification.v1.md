...
**Status:** Static inventory complete; two Tier 1 operation matrices and 21 report classifications remain unresolved
**As of:** 2026-08-08
...
| Receivables lifecycle | Available | REST entities for estimates, invoices, receipts, payments, credits, refunds, and deposits; update/delete/void support is unresolved at the individual-entity level | Tier 1 | Per-entity operation matrix, idempotency, Canadian tax fixtures |
| Payables lifecycle | Available | REST entities for purchase orders, purchases, bills, payments, and vendor credits; update/delete/void support is unresolved at the individual-entity level | Tier 1 | Per-entity operation matrix, idempotency, Canadian tax fixtures |
...
Four product reports not named in that endpoint table have an initial manual/conditional classification: Project Profitability, Budget vs Actual, Canadian Sales Tax Liability, and Reconciliation Reports. The remaining 21 plan-required rows are explicitly `unknown` pending exact product/API/manual classification. Inventory completeness is therefore not evidence completeness.
...
- Per-entity void/update/delete differences inside composite sales, payables, and cash capabilities.
