...
3. Read only the task-relevant source files from `references/repo-map.md`.
4. Classify the task as one of:
   ...
   - QBO/API path
   - AI plan/tool path
5. If the task touches QBO writes, OAuth, stored keys, AI plan execution, seeding, generation, issue packs, checkpoints, or startup behavior, also use `qbo-safety-review`.
6. Prefer non-mutating verification. Do not start services or run QBO scripts unless the user explicitly asked.
